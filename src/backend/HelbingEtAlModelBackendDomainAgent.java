...
import backend.DisasterSpreadNetworkDataTypes;
import input.DisasterSpreadBackendParameterLoader;
...
public class HelbingEtAlModelBackendDomainAgent extends FlowDomainAgent{
    ...
    private static final Logger logger = Logger.getLogger(HelbingEtAlModelBackendDomainAgent.class);
    
    public HelbingEtAlModelBackendDomainAgent(String experimentID,
            Time bootstrapTime,
            Time runTime){
        super(experimentID, bootstrapTime, runTime);
        this.setFlowNetworkDataTypes(new DisasterSpreadNetworkDataTypes());
    ...
    public void extractDomainParameters(){
        // do nothing
        //this.
        //this.toleranceParameter = (Double)getDomainParameters().get(DisasterSpreadBackendParameter.)
    }
    
    @Override
    public void setFlowParameters(FlowNetwork flowNetwork){
        flowNetwork.setNodeFlowType(DisasterSpreadNodeState.DAMAGE);
        flowNetwork.setNodeCapacityType(DisasterSpreadNodeState.TOLERANCE);
        //this. = ()
    }
    
    @Override
    public void loadDomainParameters(String backendParamLocation){
        DisasterSpreadBackendParameterLoader backendParameterLoader = new DisasterSpreadBackendParameterLoader(parameterColumnSeparator);
